"""Infer field metadata from local tabular files for the web generation flow."""
...
def _read_sample(path: Path, size: int = 8192) -> str:
    """Read a small text sample from a plain or gzip-compressed file."""
...
def _detect_separator(sample: str, default: str) -> str:
    """Infer the most likely delimiter from a text sample."""
...
def _first_non_empty_line(sample: str) -> str:
    """Return the first non-empty line from a text sample."""
...
def _separator_from_suffix(suffix: str) -> str:
    """Return the default delimiter implied by a filename suffix."""
    return "\t" if suffix in {".tsv", ".tab"} else ","


def _encoding_format_from_separator(separator: str) -> str:
    """Map a delimiter to the matching text MIME type."""
    if separator == "\t":
        return TSV_MIME
    return CSV_MIME
...
def _dtype_to_croissant(dtype: Any) -> str:
    """Map a pandas dtype to a simple Croissant datatype."""
